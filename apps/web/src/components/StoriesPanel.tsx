    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-sm bg-card border-l border-border shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border flex-shrink-0">
           <h2 className="font-semibold text-foreground">{t('storiesTitle')}</h2>
        ...
        <div className="px-5 py-4 border-b border-border flex-shrink-0 space-y-2 bg-muted/20">
           <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('newStoryForm')}</p>
          <input
            value={newTitle}
          ...
          >
             {creating ? t('creating') : t('createStory')}
          </button>
        </div>
                ...
                {epicStories.map(story => (
                  <div key={story.id} className="group bg-background border border-border rounded-lg">
                    {editing?.id === story.id ? (
                      ...
                      <div className="p-3 space-y-2">
                         <p className="text-xs text-muted-foreground">{t('deleteConfirmation', { title: story.title })}</p>
                        <div className="flex gap-2">
                          <button
