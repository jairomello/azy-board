...
export function teamLoadData(snapshot: DashboardSnapshot, mode: 'items' | 'points' = 'items'): ChartDatum[] { const members = snapshot.boxes.teamLoad.members.map(member => ({ name: member.userName, value: mode === 'items' ? Math.max(0, member.wipTotal - (member.blockedSubset ?? 0)) : Math.max(0, (member.wipPoints ?? 0) - (member.blockedPoints ?? 0)), pointsAvailable: member.wipPoints !== null })); const unassigned = Math.max(0, snapshot.boxes.teamLoad.unassignedWip - (snapshot.boxes.teamLoad.blockedUnassignedSubset ?? 0)); return unassigned ? [...members, { name: 'Unassigned', value: mode === 'items' ? unassigned : Math.max(0, (snapshot.boxes.teamLoad.unassignedWipPoints ?? 0) - (snapshot.boxes.teamLoad.unassignedBlockedPoints ?? 0)), pointsAvailable: snapshot.boxes.teamLoad.unassignedWipPoints !== null }] : members }
