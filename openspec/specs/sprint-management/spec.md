...
O sistema SHALL permitir criar, editar e encerrar N sprints vinculadas a um projeto, com nome, data de início e data de fim obrigatórios e status `PROPOSED`, `OPEN` ou `CLOSED`. Uma sprint nova SHALL iniciar como `PROPOSED`.
...
- **WHEN** admin encerra uma sprint `OPEN`
