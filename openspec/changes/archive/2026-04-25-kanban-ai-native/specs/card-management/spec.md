...
O sistema SHALL oferecer um toggle no board para exibir ou ocultar a estrutura de subtasks, afetando quais cards são visíveis.

#### Scenario: Toggle "Mostrar subtasks" ativado
- **WHEN** usuário ativa "Mostrar subtasks" no toolbar do board
- **THEN** board exibe apenas tasks folha (comportamento padrão — Leaf Rule)

#### Scenario: Toggle "Ocultar subtasks" ativado
- **WHEN** usuário desativa "Mostrar subtasks"
- **THEN** board exibe tasks do primeiro nível de cada story/épico, independente de terem filhos; o breadcrumb colapsa mostrando apenas o caminho até o nível visível
