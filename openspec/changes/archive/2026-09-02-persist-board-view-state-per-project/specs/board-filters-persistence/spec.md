...
O sistema SHALL armazenar o estado atual de filtros do board — incluindo `showSubtasks`, `storyDisplay`, `hideEmptyStories`, Centro de Custo, sprint, tags, versão, prioridade, status e autor — no `localStorage` sob a chave `board-filters:<projectId>`. A gravação SHALL ocorrer sempre que qualquer filtro ou opção de visualização for alterado. O modo de visualização, a densidade, o módulo ativo e os estados recolhidos de épicos, módulos e histórias SHALL ser armazenados separadamente e sempre no escopo do projeto.
