...
O sistema SHALL tratar uma STORY sem tarefas filhas (`isLeaf = true`) como um card arrastável no board Kanban quando o toggle "Histórias no board" estiver ativo. O card SHALL possuir um `columnId` e um `status` que mudam conforme o card é movido entre colunas, exatamente como uma TASK ou BUG.

#### Scenario: História folha aparece como card real na primeira coluna
- **WHEN** o toggle "Histórias no board" está ativo e uma STORY não possui nenhuma tarefa filha
...
- **WHEN** uma STORY folha é exibida no board
...
- **WHEN** uma STORY possui pelo menos uma tarefa filha (`isLeaf = false`)
