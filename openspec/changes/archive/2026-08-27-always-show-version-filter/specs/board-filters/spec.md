...
O sistema SHALL exibir filtros de conteúdo em um painel acionado pelo botão `Filtros` da command bar. O painel SHALL conter Squad, Módulo, Sprint, Responsável, Autor, Versão, Prioridade, Status, tipos e tags. O controle de Versão SHALL permanecer visível mesmo quando o projeto não possuir versões, exibindo `Sem versão` como estado vazio e `Nenhuma versão cadastrada` como indicação não selecionável. Os controles de visualização, incluindo Mostrar subtasks, alternância de histórias entre lanes e cards, Hierarquia/Abas, Expandir tudo, Recolher tudo e ocultação de lanes vazias, SHALL estar no painel separado acionado pelo botão `Opções`. O estado SHALL ser persistido no `localStorage` e o modo de módulos SHALL ser persistido por projeto e restaurado nas visitas subsequentes ao board.
...
#### Scenario: Filtro por versão sem versões cadastradas
- **WHEN** o usuário abre o painel Filtros em um projeto sem versões cadastradas
- **THEN** o controle Versão permanece visível, seleciona `Sem versão` por padrão e indica `Nenhuma versão cadastrada`
